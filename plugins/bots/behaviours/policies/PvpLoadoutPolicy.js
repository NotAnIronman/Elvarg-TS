...
function chooseHybridWearSet() {
  return {
    head: choose([ItemIdentifiers.COIF, ItemIdentifiers.HELM_OF_NEITIZNOT, ...MAGE_HATS]),
    cape: choose(MAGE_CAPES),
    body: choose(HYBRID_MAGE_TOPS),
...
function buildGeneratedPreset(player, state) {
  ...
  const preset = new Presetable(
    name,
    archetype.inventory(magicPackage, profile, state, player),
    archetype.equipment(magicPackage, profile, state, player),
    stats,
    magicPackage?.spellbook ?? archetype.spellbook ?? MagicSpellbook.NORMAL,
    true,
