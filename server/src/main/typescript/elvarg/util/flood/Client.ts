...
export class Client {
  ...
  public attemptLogins() {
    ...
    if (response === 0) {
      this.socketStream.flushInputStream(
        new Uint8Array(this.incoming.payload),
        8
      );
      ...
      this.socketStream.queueBytes(
        this.login.currentPosition,
        new Uint8Array(this.login.payload)
      );
  ...
  public process() {
    if (this.loggedIn) {
      ...
      if (this.pingCounter++ >= 25) {
        ...
        if (this.socketStream != null) {
          this.socketStream.queueBytes(
            this.outgoing.bufferLength(),
            this.outgoing.getBuffer()
          );
        }
        this.pingCounter = 0;
      }
