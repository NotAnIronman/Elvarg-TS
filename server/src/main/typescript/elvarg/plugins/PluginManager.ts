    ...
    private static groundItemInteractionHooks: PluginHook<PluginGroundItemInteractionEvent>[] =
      ...
      private static interfaceActionClickHooks: PluginHook<PluginInterfaceActionClickEvent>[] =
        ...
        private static createApi(pluginName: string): PluginApi {
          ...
          const resolveNpcActionOp = (npcId: number, label: string): number | null => {
            const actions = NpcDefinition.forId(npcId)?.getActions() ?? [];
            const normalized = label.trim().toLowerCase();
            const index = actions.findIndex(
              (action) => action?.trim().toLowerCase() === normalized
            );
            // Cache actions[] is 0-indexed (actions[0] = op1, actions[1] = op2, ...),
            // matching the real OSRS client-to-server OPNPC1-OPNPC5 opcodes.
          ...
          return {
            ...
            // "Talk-to") instead of a hardcoded op number. The same label can sit
            // at a different op index per NPC (real OSRS NPCs are not consistent
            // about this), so each npcId is resolved independently against its
            // own cache-derived action list (NpcDefinition.getActions()) rather
            // than assuming one op number applies to every id in the list.
            onNpcAction: (npcIds, label, handler) => {
              if (typeof label !== "string" || !label.trim()) {
                console.warn(`[plugins] ${pluginName} called onNpcAction with an invalid label`);
                return;
              }
              const normalized = Array.isArray(npcIds) ? npcIds : [npcIds];
              const byOp = new Map<number, number[]>();
              for (const npcId of normalized) {
                if (!Number.isInteger(npcId) || npcId < 0) continue;
                const op = resolveNpcActionOp(npcId, label);
                if (op === null) {
                  console.warn(
                    `[plugins] ${pluginName} onNpcAction("${label}") - NPC ${npcId} has no ` +
                    `matching menu option (actions: ${JSON.stringify(NpcDefinition.forId(npcId)?.getActions() ?? [])})`
                  );
                  continue;
                }
              ...
              for (const [op, ids] of byOp) {
                registerNpcClickHook(op, ids, handler, label);
              }
            },
            // Debugging/authoring helper - lets a plugin (or a one-off ::command)
            // print an NPC's real cache-defined option order instead of guessing.
            getNpcActions: (npcId: number) => NpcDefinition.forId(npcId)?.getActions() ?? [],
